...
test("key values are populated", (t) => {
  ...
  assert.equal(
    (data.Screenshots.legacySyntheticScreenshots?.length ?? 0) > 2,
    true
  );
...
test("numeric values are set and look legit", (t) => {
  ...
  for (const datum of shouldBeNumbers) {
    ...
    if (typeof datum !== "number") throw new Error();
...
test("string values are set and look legit", (t) => {
  ...
  for (const datum of shouldBeStrings) {
    ...
    if (typeof datum !== "string") throw new Error();
...
test("insights look ok", (t) => {
  ...
  const simplified = Object.fromEntries(
    values.map((v, i) => [
      entityNames[i],
      { transferSize: v.transferSize, mainThreadTime: v.mainThreadTime },
    ])
  );
  ...
  const expected = {
    ahfhijdlegdabablpippeagghigmibma: {
      transferSize: 0,
      mainThreadTime: 3.953000009059906,
    },
    "paulirish.com": {
      transferSize: 142142,
      mainThreadTime: 17.631999850273132,
    },
    cjpalhdlnbpafiamejdnhcphjbkeiagm: {
      transferSize: 0,
      mainThreadTime: 4.081999972462654,
    },
    jinjaccalgkegednnccohejagnlnfdag: {
      transferSize: 0,
      mainThreadTime: 1.2049999833106995,
    },
    noondiphcddnnabmjcihcjfbhfklnnep: {
      transferSize: 0,
      mainThreadTime: 2.2430000007152557,
    },
    "Google Tag Manager": {
      transferSize: 0,
      mainThreadTime: 0.2629999965429306,
    },
    ...
    bknnlbamapndemiekhkcnmdclnkijlhb: {
      transferSize: 0,
      mainThreadTime: 0.601999968290329,
    },
    kljjfejkagofbgklifblndjelgabcmig: {
      transferSize: 0,
      mainThreadTime: 0.27699998021125793,
    },
    nffaoalbilbmmfgbnbgppjihopabppdk: {
      transferSize: 0,
      mainThreadTime: 1.246999979019165,
    },
    obadmbiiipafnncogfkdfionggeckfia: {
      transferSize: 0,
      mainThreadTime: 1.696999967098236,
    },
  };
