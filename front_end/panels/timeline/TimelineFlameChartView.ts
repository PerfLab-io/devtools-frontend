...
export class TimelineFlameChartView extends Common.ObjectWrapper.eventMixin<EventTypes, typeof UI.Widget.VBox>(
  ...
  constructor(delegate: TimelineModeViewDelegate) {
    ...
    this.detailsView.addEventListener(TimelineTreeView.Events.TREE_ROW_HOVERED, node => {
      // INFO: Removing the experiment check
    ...
    document.getElementById('-blink-dev-tools')?.addEventListener(HightlightEventAsOvelay.eventName, (event: HightlightEventAsOvelay) => {
      this.setOverlays(createOverlayFnForEvent(event.detail.phases, event.detail.entry)(), {updateTraceWindow: true});
    });
  }
  ...
  setInsights(
    ...
    this.#updateDetailViews();
    document.getElementById('-blink-dev-tools')?.dispatchEvent(new TraceInsightsChange({
      insights: this.#traceInsightSets,
