...
export function matchEvents(unpairedEvents: Types.Events.PairableAsync[]): Map<string, MatchingPairableAsyncEvents> {
  ...
  for (const event of unpairedEvents) {
    ...
    if (isStartEvent) {
      ...
      if(Types.Events.isTraceEventAnimationFrameInstant(event)){
        ...
        // @ts-ignore
        otherEventsWithID.begin = event as Types.Events.PairableAsyncInstant;
      }
    }
  }
  return matchedPairs;
...
export function createSortedSyntheticEvents<T extends Types.Events.PairableAsync>(
  ...
  for (const [id, eventsTriplet] of matchedPairs.entries()) {
    ...
    if (Types.Events.isTraceEventAnimationFrameInstant(beginEvent)
        || Types.Events.isTraceEventAnimationFramePaint(beginEvent)
        || Types.Events.isTraceEventAnimationFrameScript(beginEvent)) {
    ...
    if (Types.Events.isTraceEventAnimationFrame(beginEvent)) {
